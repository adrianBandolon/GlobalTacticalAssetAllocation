...
qstratRank <- function(symbols, init.equity=100000, top.N=1, 
        ...
        # (i.e. trades the #1 ranked symbol if N=1)
        stratRank <- add.signal(strategy=stratRank, 
                                name="sigThreshold", 
                                arguments=list(threshold=N, 
                                               column="Rank", 
                                               relationship="gt", 
        ...
        stratRank <- add.rule(strategy=stratRank, name='ruleSignal', 
                              arguments = list(sigcol="Rank.lte.N", 
                                               sigval=TRUE, 
                                               orderqty=max.size, 
                                               ordertype='market', 
                                               orderside='long', 
        ...
        # add exit rule
        stratRank <- add.rule(strategy = stratRank, name='ruleSignal', 
                              arguments = list(sigcol="Rank.gt.N",
                                               sigval=TRUE, 
                                               orderqty='all', 
                                               ordertype='market', 
                                               orderside='long', 
        ...
        for(symbol in symbols){ 
                addPosLimit(port.st, symbol, initDate, maxpos=max.size, longlevels=max.levels) 
        }
        ...
        stats <- tradeStats(port.st)[ ,c('Net.Trading.PL','Max.Drawdown',
                                         'Num.Trades', 'Profit.Factor',
                                         'Std.Dev.Trade.PL', 'Largest.Winner',
                                         'Largest.Loser', 'Max.Equity',
                                         'Min.Equity', 'Ann.Sharpe')]
