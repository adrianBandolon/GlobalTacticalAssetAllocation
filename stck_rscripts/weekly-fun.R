...
weeklyAd <- function(x){
        ...
        sym <- sub("\\..*$", "", names(x)[1])
        Ad(to.weekly(x, indexAt = 'lastof', drop.time = TRUE, name = sym))
}

##### monthlyReturns function #####
weeklyReturns <- function(symbols) {
        ...
        ROC(x = weeklyPrices(symbols), n = 1, type = "discrete", na.pad = TRUE)
}

##### monthlyPrices function #####
weeklyPrices <- function(symbols) {
        # The function takes a character vector of symbols loaded into
        ...
        do.call(merge, lapply(list.sym, weeklyAd))
}