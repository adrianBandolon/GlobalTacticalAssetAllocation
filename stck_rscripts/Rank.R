...
rowRank <- function(x){
        ...
        as.xts(t(apply(-x, 1, rank, na.last = "keep")), dateFormat="Date")
}

#Use the supplied TTR::ROC function for a straight ROC computation

#####Ranking based on a single period (ie. 1 month)#####
singleROC <- function(x, n = 1){
        # Computes the returns based on 1 period (1 month)returns
        #
        #  x   : xts object of prices
        
        #roc1 <- CalculateReturns(x, method = "discrete")
        roc1 <- ROC(x, n = n[1], type = "discrete")
        as.xts(t(apply(-roc1, 1, rank, na.last = "keep")), dateFormat="Date")
}

##### ave3ROC #####
ave3ROC <- function(x, n=c(1, 3, 6, 9, 12)){
        ...
        roc4 <- ROC(x, n = n[4], type = "discrete")
        roc5 <- ROC(x, n = n[5], type = "discrete")
        ave <- (roc1 + roc2 + roc3 + roc4 + roc5)/5
        rowRank(ave)
}
